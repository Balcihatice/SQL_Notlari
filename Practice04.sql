...
--kusuratini virgulden sonra 2 deger gormek icin.
...
select count(marka) arac_marka from arac;
...
select *from meslekler
where isim='Ali' order by maas desc;
...
--offset= atlama yapmak icin kullanilir
...
--1.Yol -->> left join
select film_name,category,actor_name from filmler  left join 
aktorler 
on filmler.film_id=aktorler.film_id; 
--on ifadesiyle tablolari birbirine bagladik

--2.Yol -->> right join

select film_name,category,actor_name from aktorler  right join 
filmler 
on filmler.film_id=aktorler.film_id; 
/*
